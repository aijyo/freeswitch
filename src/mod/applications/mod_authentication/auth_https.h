...
const char* auth_session_create(switch_core_session_t* session, const char* clientId, uint32_t* http_code, char** desc);
switch_status_t auth_conference_join(switch_core_session_t* session, const char* str_account
	, const char* str_passwd, const char* token, uint32_t* http_code, char** desc);
