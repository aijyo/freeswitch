...
std::string MacEngine::getMdString(BYTE *text, int text_len, BYTE *key, int key_len)
{
	...
	unsigned char strDigest[SHA1_DIGEST_LENGTH];

	memset(strDigest, 0, SHA1_DIGEST_LENGTH);
	CSHA1::GetHash((UINT_8 *)strDigest);
	return base64_encode(strDigest, 20 /* SHA 1 digest is 160 bits */);
}
